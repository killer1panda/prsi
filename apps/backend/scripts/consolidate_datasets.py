...
def load_collective_violence(filepaths):
    ...
    result = pd.DataFrame(
        {
            "text": df.get("tweetid", pd.Series([None] * len(df))).astype(
                str
            ),  # tweetid as placeholder
            "doom_score": df["doom_score"],
...
def load_meme_datasets(filepaths):
    ...
    print(
        f"  → Loaded {len(result)} memes, {len(result[result['image_bytes'].notna()])} with images"
    )
    return result
...
def consolidate_all_datasets(data_dir):
    ...
    if not cyberbullying_files:
        # Try HPC path
        cyberbullying_files = list(
            Path("/home/vivek.120542/cyberbullying-instagram-tiktok").glob("*.parquet")
        )
    ...
    meme_files = (
        list(data_dir.glob("*meme*.parquet"))
    ...
    print(
        f"Positive samples (doom > 0.5): {(combined['doom_score'] > 0.5).sum():,} ({(combined['doom_score'] > 0.5).mean()*100:.1f}%)"
    )
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="/workspace/data/processed/unified_train.parquet",
