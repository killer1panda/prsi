...
def load_cyberbullying(parquet_dir: str) -> pd.DataFrame:
    ...
    df["doom_score"] = df[label_col].apply(
        lambda x: (
            1.0
            if any(term in str(x).lower() for term in ["bully", "harass", "severe", "high"])
            else 0.0
        )
    )
    ...
    return (
        df[[text_col, "doom_score", "source", "modality"]]
...
def load_cancelled_brands(csv_path: str) -> pd.DataFrame:
    ...
    df["doom_score"] = df[label_col].apply(
        lambda x: (
            1.0
            if any(term in str(x).lower() for term in ["yes", "true", "1", "cancelled"])
            else 0.0
        )
    )
...
def load_jigsaw(csv_path: str) -> pd.DataFrame:
    ...
    return (
        df[["comment_text", "doom_score", "source", "modality"]]
...
def consolidate_datasets(data_dir: str, output_dir: str):
    ...
    # Load CADE
    cade_files = list(data_path.glob("*cade*.parquet")) + list(
        (data_path / "CADE_aequa" / "data").glob("*.parquet")
    )
    if cade_files:
    ...
    print(
        f"Doom score range: [{combined['doom_score'].min():.2f}, {combined['doom_score'].max():.2f}]"
    )
    ...
    combined["doom_bucket"] = pd.cut(
        combined["doom_score"], bins=[-0.01, 0.3, 0.7, 1.01], labels=["low", "mid", "high"]
    )

    from sklearn.model_selection import train_test_split

    train_df, temp_df = train_test_split(
        combined, test_size=0.3, stratify=combined["doom_bucket"], random_state=42
    )
    val_df, test_df = train_test_split(
        temp_df, test_size=0.5, stratify=temp_df["doom_bucket"], random_state=42
    )
