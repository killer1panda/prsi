...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s [%(levelname)s] [Rank %(rank)s] %(name)s: %(message)s"
)
...
class HPCTrainer:
    ...
    def _emergency_checkpoint(self):
        ...
        if self.model and self.optimizer:
            ...
            self.checkpoint_manager.save_checkpoint(state, self.current_step, is_epoch_end=False)
    ...
    def setup_model(self, model: torch.nn.Module):
        ...
        if self.config.use_deepspeed and HAS_DEEPSPEED:
            # DeepSpeed ZeRO-3
            ds_config = {
                "train_batch_size": self.config.batch_size_per_gpu
                * get_world_size()
                * self.config.gradient_accumulation_steps,
                ...
                "fp16": {"enabled": self.config.use_amp and self.config.amp_dtype == "float16"},
                "bf16": {"enabled": self.config.use_amp and self.config.amp_dtype == "bfloat16"},
                "zero_optimization": {
                    "stage": 3,
                    "offload_optimizer": {"device": "cpu" if self.config.cpu_offload else "none"},
                    "offload_param": {"device": "cpu" if self.config.cpu_offload else "none"},
                },
            }
            ...
            mp_policy = MixedPrecision(
                ...
                reduce_dtype=(
                    torch.bfloat16 if self.config.amp_dtype == "bfloat16" else torch.float32
                ),
        ...
        logger.info(
            f"Model setup complete. Using: {'DeepSpeed' if self.config.use_deepspeed else 'FSDP' if self.config.use_fsdp else 'DDP'}"
        )
    ...
    def setup_data(self, train_dataset: Dataset, val_dataset: Optional[Dataset] = None):
        ...
        self.train_loader = DataLoader(
            ...
            prefetch_factor=(
                self.config.dataloader_prefetch_factor if self.config.num_workers > 0 else None
            ),
        ...
        logger.info(
            f"Data setup: {len(self.train_loader)} batches/epoch, {total_steps} total steps"
        )
