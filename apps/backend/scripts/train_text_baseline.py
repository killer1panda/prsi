...
def find_latest_parquet(data_dir: str) -> tuple:
    ...
    train_files = (
        sorted(data_path.glob("train_*.parquet"))
        + sorted(data_path.glob("train_split_*.parquet"))
        + sorted(data_path.glob("*train*.parquet"))
    )
    val_files = (
        sorted(data_path.glob("val_*.parquet"))
        + sorted(data_path.glob("validation_*.parquet"))
        + sorted(data_path.glob("*val*.parquet"))
    )
    test_files = sorted(data_path.glob("test_*.parquet")) + sorted(data_path.glob("*test*.parquet"))
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="models/text_baseline", help="Model output directory"
    )
    ...
    train_dataset = DoomTextDataset(
        train_df["text"].tolist(), train_df["doom_score"].tolist(), tokenizer, args.max_length
    )
    val_dataset = DoomTextDataset(
        val_df["text"].tolist(), val_df["doom_score"].tolist(), tokenizer, args.max_length
    )
    test_dataset = DoomTextDataset(
        test_df["text"].tolist(), test_df["doom_score"].tolist(), tokenizer, args.max_length
    )
    ...
    for epoch in range(args.epochs):
        ...
        print(f"\nResults:")
        print(
            f"  Train Loss: {train_loss:.4f} | RMSE: {train_metrics['rmse']:.4f} | Acc: {train_metrics['accuracy']:.4f}"
        )
        print(
            f"  Val   Loss: {val_loss:.4f} | RMSE: {val_metrics['rmse']:.4f} | Acc: {val_metrics['accuracy']:.4f} | AUC: {val_metrics.get('auc', 'N/A')}"
        )
        print(
            f"  Test  Loss: {test_loss:.4f} | RMSE: {test_metrics['rmse']:.4f} | Acc: {test_metrics['accuracy']:.4f} | AUC: {test_metrics.get('auc', 'N/A')}"
        )
