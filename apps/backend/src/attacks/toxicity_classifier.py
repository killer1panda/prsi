...
class ProductionToxicityClassifier:
    ...
    async def predict(self, text: str) -> ToxicityResult:
        ...
        if self.use_ensemble:
            ...
            if self.perspective_api_key:
                score, confidence, categories, flagged_tokens = await self._perspective_predict(
                    text
                )
        ...
        if self.redis and not result.cache_hit:
            try:
                await self.redis.setex(cache_key, self.cache_ttl, json.dumps(asdict(result)))
            except Exception as e:
    ...
    async def _ensemble_predict(
        ...
        # Weighted ensemble
        total_weight = sum(self.ensemble_weights.get(k, 0) for k in predictions.keys())
        if total_weight == 0:
            return self._rule_based_predict(text)

        weighted_score = (
            sum(self.ensemble_weights.get(k, 0) * v[0] for k, v in predictions.items())
            / total_weight
        )

        weighted_conf = (
            sum(self.ensemble_weights.get(k, 0) * v[1] for k, v in predictions.items())
            / total_weight
        )

        # Merge categories
        merged_categories = {}
        for cat in set().union(
            *[c.keys() for c in all_categories.values()] if all_categories else []
        ):
    ...
    async def _hf_predict(self, text: str) -> Tuple[float, float, Dict[str, float], List[str]]:
    ...
    def _rule_based_predict(self, text: str) -> Tuple[float, float, Dict[str, float], List[str]]:
        """
    ...
    async def _check_rate_limit(self):
        ...
        if self._rate_limit_tokens < 1:
            raise HTTPException(status_code=429, detail="Toxicity API rate limit exceeded")
