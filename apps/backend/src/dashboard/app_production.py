...
with tab2:
    ...
    if simulate_btn and target_text:
        ...
        if result and "variants" in result:
            ...
            cols[1].metric(
                "Max Uplift",
                f"+{max(v['doom_uplift'] for v in variants):.1f}",
                delta=f"+{max(v['doom_uplift'] for v in variants):.1f}",
            )
        ...
        try:
            ...
            if pyg_data.num_nodes > 0 and pyg_data.num_edges > 0:
                ...
                user_ids = (
                    ...
                    else list(range(pyg_data.num_nodes))
                )
                ...
                node_df = pd.DataFrame(
                    {
                        ...
                        "size": (
                            (user_df["followers"].fillna(0).clip(upper=1e6) / 1e4 + 5).tolist()
                            if "followers" in user_df.columns
                            else [10] * pyg_data.num_nodes
                        ),
                        "doom_score": (
                            (user_df["avg_toxicity"].fillna(0) * 100).tolist()
                            if "avg_toxicity" in user_df.columns
                            else [50] * pyg_data.num_nodes
                        ),
                ...
                network_df = node_df
                st.caption(
                    f"Live graph: {pyg_data.num_nodes} users, {pyg_data.num_edges} interactions — Neo4j"
                )
        except Exception as e:
            st.caption(
                f"⚠️ Neo4j offline or GraphExtractor error ({type(e).__name__}). Showing demo layout."
            )
...
with tab4:
    ...
    if _TRADEOFF_CSV.exists():
        try:
            ...
            if "epsilon" in tradeoff_df.columns:
                tradeoff_df["epsilon_numeric"] = pd.to_numeric(
                    tradeoff_df["epsilon"], errors="coerce"
                )
                tradeoff_df["epsilon"] = tradeoff_df["epsilon"].astype(str)
                st.caption(
                    f"✅ Live data from `{_TRADEOFF_CSV}` ({len(tradeoff_df)} epsilon levels)"
                )
            else:
    ...
    fig.update_layout(title="Privacy-Utility Tradeoff Curve", height=500, hovermode="x unified")
    ...
    **Current Configuration (ε=1.0):**
    ...
    for col in noise_comparison.columns:
        fig_noise.add_trace(
            go.Scatter(y=noise_comparison[col], mode="lines", name=col, opacity=0.8)
        )
