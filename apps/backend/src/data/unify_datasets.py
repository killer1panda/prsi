...
class DataUnifier:
                            ...
                            samples.append(
                                UnifiedSample(
                                    ...
                                    sample_id=self._generate_sample_id(
                                        text, DataSource.CYBERBULLYING.value
                                    ),
            ...
            try:
                ...
                for _, row in df.iterrows():
                    text = self._normalize_text(
                        str(row.get("text", "") or row.get("preprocessed_text", ""))
                    )
                    label = int(row.get("label", 0))
                    ...
                    if self._validate_sample(text, label):
                        samples.append(
                            UnifiedSample(
                                ...
                                sample_id=self._generate_sample_id(
                                    text, DataSource.HATE_SPEECH_1829.value
                                ),
                    ...
                    if self._validate_sample(text, label):
                        ...
                        samples.append(
                            UnifiedSample(
                                ...
                                sample_id=self._generate_sample_id(
                                    text, DataSource.TWEET_BLM.value
                                ),
                                platform="twitter",
                    ...
                    if text and len(text) >= self.min_text_length:
                        samples.append(
                            UnifiedSample(
                                ...
                                sample_id=self._generate_sample_id(
                                    text, DataSource.CANCELLED_BRANDS.value
                                ),
                                platform="twitter",
    ...
    def load_jigsaw_toxic_datasets(self) -> List[UnifiedSample]:
        ...
        if path.exists():
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    label = (
                        1
                        if (row.get("malignant", 0) == 1 or row.get("highly_malignant", 0) == 1)
                        else 0
                    )
                    ...
                    if self._validate_sample(text, label):
                        samples.append(
                            UnifiedSample(
                                ...
                                sample_id=self._generate_sample_id(
                                    text, DataSource.JUGSAW_TOXIC.value
                                ),
        ...
        path = (
            self.data_root
            / "hate"
            / "Social-Media-Toxic-Comments-Classification-main"
            / "data"
            / "train.csv"
        )
        if path.exists():
    ...
    def load_sexism_dataset(self) -> List[UnifiedSample]:
        ...
        path = (
            ...
            / "sexism-socialmedia-balanced.csv"
        )
        if not path.exists():
            path = (
                self.data_root
                        ...
                        if text and len(text) >= self.min_text_length:
                            ...
                            samples.append(
                                UnifiedSample(
                                    ...
                                    sample_id=self._generate_sample_id(
                                        text, DataSource.DOOM_INDEX.value
                                    ),
                                    platform="twitter",
                                    engagement_score=min(engagement, 1.0),
                                    language="en",
    ...
    def deduplicate_samples(self, samples: List[UnifiedSample]) -> List[UnifiedSample]:
        ...
        logger.info(
            f"Deduplication: removed {duplicates_removed} duplicates, kept {len(unique_samples)} samples"
        )
        return unique_samples

    def balance_datasets(
        self, samples: List[UnifiedSample], max_per_source: Optional[int] = None
    ) -> List[UnifiedSample]:
        ...
        for (source, label), group_samples in grouped.items():
            ...
            sampled = np.random.choice(len(group_samples), size=n_samples, replace=False).tolist()
    ...
    def save_to_parquet(self, samples: List[UnifiedSample], output_path: Path) -> None:
...
def main():
    ...
    parser.add_argument("--min-text-length", type=int, default=10, help="Minimum text length")
