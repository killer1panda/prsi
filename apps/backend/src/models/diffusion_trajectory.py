...
class DiTBlock1D(nn.Module):
    ...
    def __init__(self, hidden_dim: int, num_heads: int = 4, cond_dim: int = 128):
        ...
        self.cond_proj = nn.Sequential(nn.SiLU(), nn.Linear(cond_dim, hidden_dim * 2))
...
class DiffusionTrajectoryForecaster(nn.Module):
    ...
    def forward(
        self, y_noisy: torch.Tensor, t: torch.Tensor, context_embedding: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def compute_loss(self, y_0: torch.Tensor, context_embedding: torch.Tensor) -> torch.Tensor:
    ...
    def sample_trajectories(
        self, context_embedding: torch.Tensor, num_samples: int = 100, device: str = "cpu"
    ) -> Dict[str, np.ndarray]:
        """
        ...
        for step in reversed(range(self.num_diffusion_timesteps)):
            ...
            if step > 0:
                ...
                sigma = torch.sqrt(
                    beta * (1.0 - self.alphas_cumprod[step - 1]) / (1.0 - alpha_cumprod)
                )
            else:
            ...
            y = (1.0 / torch.sqrt(alpha)) * (
                y - (beta / torch.sqrt(1.0 - alpha_cumprod)) * noise_pred
            ) + sigma * z
