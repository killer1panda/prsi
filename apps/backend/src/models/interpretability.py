    ...
    def explain_graph_neighborhood(self, user_idx: int, graph_data) -> Dict:
        ...
        with torch.no_grad():
            ...
            for n_idx in neighbors[:20]:  # Limit to top 20
                sim = F.cosine_similarity(
                    user_emb.unsqueeze(0), embeddings[n_idx].unsqueeze(0), dim=-1
                ).item()
        ...
        return {
            ...
            "top_influential_neighbors": [
                {"neighbor_idx": idx, "similarity": round(sim, 4)} for idx, sim in similarities[:10]
            ],
            "explanation": (
                f"Prediction influenced by {len(neighbors)} connected users. "
                f"Top neighbor similarity: {similarities[0][1]:.3f}"
                if similarities
