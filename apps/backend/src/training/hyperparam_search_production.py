...
try:
    import optuna
    from optuna.pruners import HyperbandPruner, MedianPruner, TPESampler
...
class HyperparameterOptimizer:
    ...
    def objective(self, trial: Trial) -> float:
        ...
        try:
            ...
            logger.info(
                f"Trial {trial.number} completed: {self.config.metric}={objective_value:.4f} in {duration:.1f}s"
            )
    ...
    def run_search(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Complete: {results['complete_trials']}, Pruned: {results['pruned_trials']}, Failed: {results['failed_trials']}"
        )
...
def main():
    ...
    parser.add_argument(
        "--study-name", type=str, default="doom-index-hp-search", help="Name of the Optuna study"
    )
    ...
    parser.add_argument(
        "--direction", type=str, default="maximize", choices=["minimize", "maximize"]
    )
