...
class ContrastiveHead(nn.Module):
    ...
    def __init__(self, hidden_dim: int, projection_dim: int):
        ...
        self.projector = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, projection_dim)
        )
...
class SemiSupervisedDoomClassifier:
    ...
    def __init__(self, config: ModelConfig, device: Optional[str] = None):
        ...
        if config.strategy == SemiSupervisedStrategy.CONTRASTIVE_PRETRAIN:
            ...
            self.contrastive_head = ContrastiveHead(hidden_dim, config.projection_dim).to(
                self.device
            )
    ...
    def generate_pseudo_labels(
        self, unlabeled_texts: List[str], batch_size: int = 64
        ...
        logger.info(
            f"Generated {len(pseudo_labeled_texts)} pseudo-labels ({len(pseudo_labeled_texts)/len(unlabeled_texts)*100:.1f}% of data)"
        )
    ...
    def train_self_training(
        ...
        for iteration in range(n_iterations):
            ...
            train_dataset = DoomIndexDataset(
                labeled_texts, labeled_labels, self.tokenizer, self.config.max_length
            )
    ...
    def finetune_with_pretrained_weights(
        ...
        self.classifier = AutoModelForSequenceClassification.from_pretrained(
            pretrained_path, num_labels=self.config.num_labels
        )
        self.classifier.to(self.device)
...
def main():
    ...
    if args.strategy == "supervised":
        train_dataset = DoomIndexDataset(
            train_texts, train_labels, trainer.tokenizer, config.max_length
        )
