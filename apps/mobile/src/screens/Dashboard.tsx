...
import { fetchRiskData, fetchOverallRisk } from '../api/client';

export default function Dashboard() {
  const [riskFactors, setRiskFactors] = useState([]);
  const [overallRisk, setOverallRisk] = useState({ score: 0, status: 'Loading...' });

  useEffect(() => {
    const loadData = async () => {
      const risks = await fetchRiskData();
      const overall = await fetchOverallRisk();
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.mainCard}>
          ...
          <View style={styles.scoreContainer}>
            <Text style={styles.scoreText}>{overallRisk.score}</Text>
            <Text style={styles.scoreMax}>/100</Text>
          </View>
          <Text style={styles.mainCardStatus}>{overallRisk.status}</Text>
        </View>

        <Text style={styles.sectionTitle}>Key Risk Factors</Text>
        
        {riskFactors.map((item: any) => (
