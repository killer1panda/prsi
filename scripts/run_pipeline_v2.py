...
class DoomTrajectoryForecaster:
    ...
    def fit(self, historical_scores: List[Tuple[datetime, float]]):
        ...
        logger.info(
            f"Fitted trajectory model: slope={self.model.coef_[0]:.3f}, intercept={self.model.intercept_:.3f}"
        )
        return self
    ...
    def predict_trajectory(self, horizon_days: int = 7, current_score: float = None) -> Dict:
        ...
        return {
            ...
            "trend": (
                "increasing"
                if self.model.coef_[0] > 0.5
                else "decreasing" if self.model.coef_[0] < -0.5 else "stable"
            ),
...
class PipelineOrchestrator:
    ...
    def run_stage(self, stage: PipelineStage, func: Callable, *args, **kwargs) -> StageResult:
        ...
        if self.config.resume_from and stage.value < self.config.resume_from:
            ...
            return StageResult(
                stage=stage.value, success=True, duration_seconds=0, artifacts={"skipped": True}
            )
    ...
    def stage_report_generation(self) -> Dict:
        ...
        with open(summary_path, "w") as f:
            ...
            f.write("=" * 60 + "\n\n")
            f.write(
                f"Total Duration: {report['pipeline_execution_summary']['total_duration_seconds']:.2f}s\n"
            )
            f.write(
                f"Stages Completed: {report['pipeline_execution_summary']['stages_completed']}/{len(PipelineStage)}\n\n"
            )
    ...
    def run_forecasting(self, username: str) -> Dict:
        ...
        if trajectory.get("critical_threshold_crossing"):
            logger.warning(
                f"⚠️  CRITICAL: Doom score may exceed 80 in {trajectory['critical_threshold_crossing']} days!"
            )
...
Examples:
  ...
  python scripts/run_pipeline_v2.py --resume-from stage_3_features
    ...
    parser.add_argument(
        "--forecast", action="store_true", help="Enable doom trajectory forecasting"
    )
    parser.add_argument(
        "--username", type=str, default="demo_user", help="Username for forecasting"
    )
    ...
    if args.forecast:
        ...
        print(
            f"Current Score: {forecast_results['trajectory'].get('current_score', 'N/A'):.1f}/100"
        )
        ...
        if forecast_results["trajectory"].get("critical_threshold_crossing"):
            print(
                f"⚠️  WARNING: May reach critical doom (80+) in {forecast_results['trajectory']['critical_threshold_crossing']} days!"
            )
        print("=" * 60)
