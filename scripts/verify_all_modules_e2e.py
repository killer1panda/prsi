...
def verify_all():
    ...
    from src.data.db_connectors import MongoDBConnector, InMemoryCollection

    preprocessor = DataPreprocessor()
    clean_sample = preprocessor.clean_text("OMG this is so crazy!!! Check out https://t.co/xyz123 @user #cancel")
    assert len(clean_sample) > 0, "Clean text failed"

    raw_posts = [
        {"author_id": "u1", "body": "I cannot stand this behavior #boycott", "created_utc": 1700000000},
        {"author_id": "u2", "body": "Bhai kya chal raha hai ye sab? #drama", "created_utc": 1700003600},
    ]
    ...
    assert "polarity" in sent_res or "sentiment" in sent_res or "compound" in sent_res or "score" in sent_res, "Sentiment failed"
    ...
    density_map = extractor.compute_echo_chamber_density(["u1", "u2", "u3", "u4", "u5"], sample_edges)
    ...
    stream_posts = [
        {"image_url": "http://test1.jpg", "meme_data": {"is_meme": True, "virality_score": 0.88, "meme_type": "soyjak"}},
        {"image_url": "http://test2.jpg", "meme_data": {"is_meme": True, "virality_score": 0.72, "meme_type": "gigachad"}},
        {"image_url": "http://test3.jpg", "meme_data": {"is_meme": False, "virality_score": 0.15, "meme_type": "original"}},
    ]
    ...
    from src.attacks.adversarial_production import ProductionAdversarialGenerator, VisualVenomInjector
    ...
    variants = adv_gen.generate_variants("I strongly disagree with the current leadership policy", max_variants=2)
    ...
    with TestClient(app) as client:
        ...
        resp_pred = client.post(
            "/predict",
            json={"text": "This public statement is very controversial", "author_id": "test_influencer", "followers": 15000},
