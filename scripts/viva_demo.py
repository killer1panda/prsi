...
def demo_safe_post():
    ...
    try:
        ...
        console.print(
            "[dim]Interpretation: Normal positive social media post. No cancellation indicators.[/dim]"
        )
...
def demo_controversial_post():
    ...
    try:
        ...
        console.print(
            "[dim]Interpretation: Action-oriented language, high toxicity, boycott demands detected.[/dim]"
        )
    except Exception as e:
        console.print(f"[red]Error: {e}[/red]")
...
def demo_attack_simulation():
    ...
    try:
        ...
        console.print(table)
        console.print(
            "[dim]The simulator finds text mutations that maximize doom score while evading moderation.[/dim]"
        )
    except Exception as e:
        console.print(f"[red]Error: {e}[/red]")
...
def demo_batch_prediction():
    ...
    try:
        r = requests.post(f"{API_URL}/predict/batch", json={"items": items}, timeout=30)
        result = r.json()
        ...
        for pred in result.get("predictions", []):
            color = (
                "red"
                if pred["doom_score"] >= 60
                else "yellow" if pred["doom_score"] >= 40 else "green"
            )
            table.add_row(
                pred.get("id", "?"),
...
def demo_privacy_analysis():
    ...
    try:
        ...
        console.print(
            f"[blue]Differential Privacy: ε={dp.get('epsilon', 'N/A')}, δ={dp.get('delta', 'N/A')}[/blue]"
        )
...
def main():
    ...
    console.print(
        "[dim]Recommended viva flow: Show architecture diagram → Run demos → Discuss privacy/fairness[/dim]"
    )
