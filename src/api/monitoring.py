    ...
    def __init__(self, enabled: bool = True):
        ...
        if self.enabled:
            ...
            self.request_count = Counter(
                "doom_requests_total", "Total requests", ["endpoint", "status"]
            )
            ...
            self.risk_level_count = Counter(
                "doom_risk_level_total", "Count of predictions by risk level", ["level"]
            )
...
class DriftDetector:
    ...
    def compute_kl_divergence(
        self, expected: np.ndarray, actual: np.ndarray, bins: int = 20
    ) -> float:
