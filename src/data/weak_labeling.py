...
class WeakLabelingPipeline:
    ...
    def apply_labeling_functions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Weak labeling complete. Label distribution:\n{df['weak_label'].value_counts()}"
        )
    ...
    def llm_validate_subset(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        validation_df = validation_df.sample(
            min(self.config.llm_validation_sample_size, len(validation_df))
        )
        ...
        feature_cols = [
            ...
            if c.startswith("lf_") and c not in ["weak_label", "llm_label", "lf_confidence"]
        ]
        ...
        if valid_mask.sum() > 100:
            ...
            df["final_label"] = np.where(
                df["lf_confidence"] > 0.7, df["weak_label"], df["corrected_label"]
            )
        else:
...
Answer:"""
        ...
        try:
            result = (
                self.llm_pipeline(
                    prompt, max_new_tokens=20, do_sample=False, return_full_text=False
                )[0]["generated_text"]
                .strip()
                .upper()
            )
    ...
    def balance_dataset(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        sampling_strategy = {0: min(target_neg, len(negative)), 1: max(target_pos, len(positive))}
        ...
        pipeline = ImbPipeline(
            [
                (
                    "under",
                    RandomUnderSampler(
                        sampling_strategy={0: sampling_strategy[0]}, random_state=42
                    ),
                ),
        ...
        try:
            ...
            logger.info(
                f"Balanced dataset: {len(resampled_df)} samples, " f"{y_res.mean():.2%} positive"
            )
            return resampled_df
        except Exception as e:
    ...
    def _generate_report(self, df: pd.DataFrame):
        ...
        report = {
            ...
            "labeling_functions": [
                {"name": lf.name, "weight": lf.weight} for lf in self.labeling_functions
            ],
        }
